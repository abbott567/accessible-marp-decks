...
- The visually-hidden **"End of slide N"** paragraph is no longer injected into each slide footer. Every slide is already a labelled region ("Slide N: <heading>"), so screen readers announce slide boundaries anyway — the extra text was noise.
...
- `paginate: false` is now honoured: the page-number chip is visually hidden when pagination is off. The slide `aria-label`s and stable heading ids remain either way — the directive controls visible page numbers, not the accessibility machinery.
